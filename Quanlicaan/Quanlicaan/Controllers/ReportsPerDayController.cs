    ...
    public class ReportsPerDayController : Controller
    {
        ...
        public ActionResult ReportPerDay(DateTime? time)
        {
            ...
            else
            {
                ...
                string query = "select nv.*, sa.*, pb.*,(select(cOALESCE(sum(Soluong), 0))  from ChiTietSuatAn ct join SuatAn sa on ct.IDSuatAn = sa.ID where ct.IDUser = nv.ID and ct.IDCaan = '1')soluongca1 ,(select(cOALESCE(sum(Soluong), 0))  from ChiTietSuatAn ct  where ct.IDCaan = '1' )Tongsoluongca1 ,(select(cOALESCE(sum(Soluong), 0))  from ChiTietSuatAn ct join SuatAn sa on ct.IDSuatAn = sa.ID where ct.IDUser = nv.ID and ct.IDCaan = '2' )soluongca2,(select(cOALESCE(sum(Soluong), 0))  from ChiTietSuatAn ct  where ct.IDCaan = '2' )Tongsoluongca2 ,(select(cOALESCE(sum(Soluong), 0)) from ChiTietSuatAn ct join SuatAn sa on ct.IDSuatAn = sa.ID where ct.IDUser = nv.ID and ct.IDCaan = '1002' )soluongca3, (select(cOALESCE(sum(Soluong), 0))  from ChiTietSuatAn ct  where ct.IDCaan = '1002' )Tongsoluongca3 ,(select(cOALESCE(sum(Soluong), 0))  from ChiTietSuatAn ct join SuatAn sa on ct.IDSuatAn = sa.ID where ct.IDUser = nv.ID  )Tongsoluong from NhanVien nv join SuatAn sa on nv.ID = sa.IDUser JOIN PhongBan pb on nv.IDPhongBan = pb.ID  ";
                command = new SqlCommand(query, conn);
